...
def process_marker_data(marker_name, data_path, output_path):
    ...
    # 检查是否存在汇总的文件夹，不在则创建
    marker_summary_path = os.path.join(output_path, "summary")
    if not os.path.exists(marker_summary_path):
        os.makedirs(marker_summary_path, exist_ok=True)
    ...
    for idx, image_path in enumerate(image_files, start=1):
        ...
        summary_file_path = os.path.join(marker_summary_path, summary_filename)
        # 复制文件到汇总的文件夹
        shutil.copy2(image_path, summary_file_path)

    print(f"    成功复制 {total_images} 张图片到 {marker_summary_path}")
...
def main():
    ...
    # 设置路径,路径上类似于D:\Pycharm\Projects\MachineLearning\markerrecognition\src\..\data\temporarydata
    data_path = os.path.join(script_dir, "..", "data", "temporarydata")
